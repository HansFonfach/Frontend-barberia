    ...
    >
      {[
        ...
        >
          ...
          />
          <small
            style={{ fontSize: "0.65rem", color: "#8898aa", fontWeight: 600 }}
          >
            {title}
          </small>
          <small style={{ fontSize: "0.6rem", color: "#adb5bd" }}>
            {color}
          </small>
        </div>
      ))}
...
const ConfiguracionEmpresa = () => {
  ...
  const handleUploadLogo = async (file) => {
    ...
    try {
      ...
      const res = await postLogoEmpresa(empresa._id, formData);
      set("logo", res.data.logo);
      Swal.fire({
                ...
                {tab === "apariencia" && (
                  <>
                    ...
                    <Toggle
                      ...
                      hint="Las horas disponibles se anclan a bloques fijos"
                      checked={form.configuracion.usaHorasAncla}
                      onChange={(v) => set("configuracion.usaHorasAncla", v)}
                    />
                    <Toggle
                      label="Modo oscuro"
                      hint="Activa el tema oscuro en el dashboard"
                      checked={form.configuracion.modoOscuro}
                      onChange={(v) => set("configuracion.modoOscuro", v)}
                    />
                  </>
                )}
